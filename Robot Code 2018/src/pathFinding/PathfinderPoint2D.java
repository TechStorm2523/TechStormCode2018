...
public class PathfinderPoint2D{
	...
	public Obstacle checkObstacles(Map<String, Obstacle> obs, Line2D.Double path) {
		...
		for(Map.Entry<String, Obstacle> mapEntry : obs.entrySet()) {
			...
			for(Line2D.Double l : temp.lines) {
		...
		double closest = java.lang.Double.MAX_VALUE;
		double dist;
		String closestKey = null;
		Point2D.Double center;
		for(String s : keysIntersect) {
			center = obs.get(s).getCenter();
			dist = center.distance(path.getP1());
			if(dist<closest) {
				closest = dist;
				closestKey = s;
			}
